...
<script language="JavaScript">
    function writeReciept() {
        ...
        top.wRef.document.writeln(
            '<html><head><title>Order Reciept</title></head><body onLoad="self.focus()">'
            +'<center>'
        ...
        for (i = 0; i<quantity.length; i++) {
            buffer+= '<tr><td>'+quantity[i]+'</td>';
            buffer+= '<td>'+name[i]+'</td>';
            buffer+= "<td style='text-align:right;'>"+parseFloat(price[i]).toFixed(2)+'</td>';
            buffer+= "<td style='text-align:right;'>"+parseFloat(price[i]*quantity[i]).toFixed(2)+'</td> </tr>';
        }

        buffer += '</table>'

        buffer+='Total: ' + parseFloat(total).toFixed(2) + '<br>';

        buffer += '</center></body></html>'
